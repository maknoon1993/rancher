...
func checkImage(image string) error {
	...
	if strings.HasPrefix(image, "weaveworks") || strings.HasPrefix(image, "registry") || strings.HasPrefix(image, "busybox") {
		return nil
	}
	...
	if strings.HasSuffix(imageNameTag[0], "-") {
		return fmt.Errorf("Image [%s] has trailing '-', probably an error in image subsitution", image)
	}
	return nil
}
